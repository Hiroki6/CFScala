...
class EvalMatrixFactorization(mfModel: MatrixFactorization) {
  ...
  def calcMSE(testMFD: MFD): Double = {
    calcError(testMFD.iterator, 0.0, 0)(mfModel.predict)(calcAbsError)
  }

  def calcRMSE(testMFD: MFD): Double = {
    calcError(testMFD.iterator, 0.0, 0)(mfModel.predict)(calcSquareError)
  }

  @annotation.tailrec
  private def calcError(mfdIterator: MFDIterator, error: Double, count: Int)(predictF: (Int, Int) => Double)(errorF: (Double, Double) => Double): Double = {
    if(mfdIterator.hasNext) {
      val (userId, itemId, rate) = mfdIterator.next
      if(rate != 0.0) calcError(mfdIterator, error + errorF(rate, predictF(userId, itemId)), count+1)(predictF)(errorF)
      else calcError(mfdIterator, error, count)(predictF)(errorF)
